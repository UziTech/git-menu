...
export default {
	...
	clean(cwd, files) {
		return this.cmd(cwd, ["clean", "--quiet", "--force", "--", ...files])
			.then(result => {
				if (result !== "") {
					console.log("git clean result:", result.trimRight());
				}
			});
	},
	...
	unstage(cwd, files) {
		return this.cmd(cwd, ["reset", "HEAD", "--quiet", "--", ...files])
			.then(result => {
				if (result !== "") {
					return Promise.reject(result.trimRight());
				}
			});
	},
		...
		return this.cmd(cwd, ["branch", "--list", "--all"])
			.then(result => {
				...
				const branches = result.trimRight().split("\n").map(line => {
					const path = line.trimRight().trimLeft();
					const name = path.replace(/^\* /, "").replace(/^remotes\/origin\//, "");
	 ...
	 */
	fetch(cwd) {
		return this.cmd(cwd, ["fetch", "--all", "--quiet"])
			.then(result => {
				if (result !== "") {
					return Promise.reject(result.trimRight());
				}
	 ...
	 * @return {Promise} void
	 */
	updateIndex(cwd, files, ignore = true) {
		return this.cmd(cwd, ["update-index", "--" + (ignore ? "" : "no-") + "assume-unchanged", "--stdin"], files.join("\n"))
			.then(result => {
				if (result !== "") {
					return Promise.reject(result.trimRight());
				}
