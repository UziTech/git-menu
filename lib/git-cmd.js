...
export default {
		...
		return this.cmd(cwd, ["rev-list", "--count", "HEAD"])
			.then(result => {
				...
				return this.cmd(cwd, ["log", "-1", "--format=%B"])
					.then(_ => Promise.reject(err), logErr => {
	...
	setUpstream(cwd, remote, branch, verbose = false) {
		...
		args = args.concat("--set-upstream", remote, branch);
		return this.cmd(cwd, args);
	},
				...
				try {
					return result.trimRight()
						...
						.map(line => {
							...
							if (!lineMatch) {
								throw new Error("git status output invalid: '" + line.replace(" ", "·") + "'");
							}
							const [, code, file] = lineMatch;
							const status = this.statusFromCode(code);
							if (status === false) {
								throw new Error("Unknown code '" + line.replace(" ", "·") + "'");
							}
							return { ...status, file };
						});
				} catch (ex) {
