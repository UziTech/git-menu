...
export default {
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	commit(cwd, message, amend, files = null, verbose = false) {
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	async deleteBranch(cwd, branch, remote, force, verbose = false) {
		const verboseArg = (!verbose ? "--quiet" : "");
		const forceArg = (force ? "--force" : "");
		let result = await this.cmd(cwd, ["branch", "--delete", forceArg, verboseArg, branch]);
		if (remote) {
			const remoteVerboseArg = (verbose ? "--verbose" : "--quiet");
			const remoteResult = await this.cmd(cwd, ["push", "--delete", forceArg, remoteVerboseArg, "origin", branch]);
			if (remoteResult) {
				if (result) {
					result += "\n\n";
				}
