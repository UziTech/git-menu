...
export default {
	...
	cmd(cwd, args = [], stdin = "", includeCommand = true) {
		return new Promise((resolve, reject) => {
			let output = "";
			const git = atom.config.get("context-git.gitPath");
	 ...
	 * Get Log
	 * @param {string} cwd Current Working Directory
	 * @param {int} number Number of commits
	 * @param {string} [offset=0] Offset
	 * @param {string} [format="%B"] Format
	 * @return {Promise} {string|null} The last commit message or null if no commits
	 */
	async log(cwd, number, offset = 0, format = "%B") {
		try {
	 ...
	 */
	async branches(cwd) {
		const result = await this.cmd(cwd, ["branch", "--list", "--all"], "", false);
		if (result === "") {
			return [];
		}
		...
		return result.split("\n").map(line => {
			...
			const selected = branch.substring(0, 1) === "*";
