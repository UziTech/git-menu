...
export default {
	...
	async rootDir(cwd) {
		...
		return result.trimEnd().replace(/[\\/]/g, path.sep);
	},
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	createBranch(cwd, branch, remote, verbose = isVerbose()) {
		const verboseArg = (!verbose ? "--quiet" : "");
		const remoteArg = [];
		if (remote) {
			remoteArg.push("--track", `${remote}/${branch}`);
		}
		return this.cmd(cwd, ["checkout", verboseArg, "-b", branch, ...remoteArg]);
	},
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	async push(cwd, force = false, verbose = isVerbose()) {
		const tracking = [];
		const result = await this.cmd(cwd, ["status", "-b", "--porcelain"]);
		const [, branch, remote] = result.match(/^## (.+?)(?:\.\.\.(.+))?$/m);
		if (!remote) {
		...
		const verboseArg = (verbose ? "--verbose" : "--quiet");
		const forceArg = (force ? "--force" : "");
		return this.cmd(cwd, ["push", verboseArg, forceArg, ...tracking]);
	},
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	abort(cwd, merge = true) {
		const type = (merge ? "merge" : "rebase");
