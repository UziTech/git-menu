...
export default {
		...
		return new Promise((resolve, reject) => {
			...
			const bp = new BufferedProcess({
				...
				exit: code => {
					...
					let inputCmd = "> " + git + " " + args.join(" ");
					if (process.platform === 'win32') {
						output = "> " + bp.process.spawnargs[bp.process.spawnargs.length - 1].replace(/^"|"$/g, "") + "\n\n" + output;
					} else {
	...
	deleteBranch(cwd, branch, remote, force, verbose = false) {
		...
		if (remote) {
			return this.cmd(cwd, ["push", "--delete", forceArg, verboseArg, "origin", branch]);
		} else {
			return this.cmd(cwd, ["branch", "--delete", forceArg, verboseArg, branch]);
		}
	},
