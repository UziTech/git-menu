	...
	commit(event) {
		...
		this.getDirectories(filePaths)
			...
			.then(([
				...
				let commitPromise = git.add(root, filePaths)
					...
					.then(_ => {
						if (push) {
							...
							return git.push(root).then(_ => fileCount + " File" + (fileCount !== 1 ? "s" : "") + " committed & pushed");
						}
						return fileCount + " File" + (fileCount !== 1 ? "s" : "") + " committed";
					});
