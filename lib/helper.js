...
export default {
	...
	async checkGitLock(root, retry = 100) {
		...
		let confirmButton, removeLock;
		if (major > 1 || minor > 24) {
			[confirmButton, removeLock] = await promisify(atom.confirm.bind(atom), {rejectOnError: false, alwaysReturnArray: true})({
				type: "error",
