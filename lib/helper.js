...
export default {
	...
	async getDirectories(filePaths) {
		const directories = await Promise.all(filePaths.map(async filePath => {
	 ...
	 * @return {Promise} {void}
	 */
	async checkGitLock(root, git, retry = 100) {
	...
	/**
	 * Reduce files to their folder if all changed files in that folder are selected
	 * @param  {string} root The root of the git repository
	 * @param  {string[]} selectedFiles The selected files to reduce
	 * @param  {Object} git git-cmd
	 * @return {Promise} {string[]} Resolves to reducedFiles
	 */
	async reduceFilesToCommonFolders(root, selectedFiles, git) {

		const statuses = await this.getStatuses([root], git);
		const changedFiles = statuses.map(status => status.file);

		// filter out non-changed files
		let reducedFiles = selectedFiles.filter(file => changedFiles.includes(file));
		if (reducedFiles.length === changedFiles.length) {
			// TODO: should we use "." instead of root since none of the other returned paths will be absolute?
			return [root];
		}
		...
		const folders = reducedFiles.reduce((prev, file) => {
			let folder = file;
			while ((folder = path.dirname(folder) + "/") && folder !== "./" && !prev.includes(folder)) {
				prev.push(folder);
			}
		...
		let replaceFolders = folders.reduce((prev, folder) => {
			const allSelectedInChanged = changedFiles.every(file => {
