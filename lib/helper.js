...
export default {
	...
	getRootAndAllFiles(filePaths) {
		...
		return Promise.all([getFiles, this.getRoot(filePaths)]);
	},
	 ...
	 */
	getRootAndFilesStatuses(filePaths) {

		if (this.getUnsavedFiles(filePaths).length > 0) {
			return Promise.reject("Save files before running git.");
		}

		return Promise.all([this.getStatuses(filePaths), this.getRoot(filePaths)]);
	},

	getRootAndFiles(filePaths) {
		return this.getRootAndFilesStatuses(filePaths).then(([statuses, root]) => {
