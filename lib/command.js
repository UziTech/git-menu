...
function filesInFilePaths(filePaths, files) {
	return files.filter(file => filePaths.some(filePath => file.startsWith(filePath)));
}
...
const commands = {
	...
	discardChanges(event, statusBar) {
		return getFiles(event.target).then(([files, root]) => {
			let untrackedFiles = [];
			let trackedFiles = [];
	...
	discardAllChanges(event, statusBar) {
		return commands.discardChanges({ target: null }, statusBar);
	},

	addToLastCommit(event, statusBar) {
		return getFiles(event.target).then(([files, root]) => {
			return git.lastCommit(root)
				.then(lastCommit => {
	...
	switchBranch(event, statusBar) {
		return getFiles(event.target).then(([files, root]) => {
			return git.branches(root)
				.then(branches => {
					return new switchBranchDialog().activate(branches);
				})
				.then(branch => {
...
module.exports = (event, statusBar, command) => {
