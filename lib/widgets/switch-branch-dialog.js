...
export default class Dialog extends View {
	static content() {
		return this.div({ class: "dialog context-git", keyup: "keyup" }, () => {
			...
			this.div({ class: "body" }, () => {
				...
				this.select({ class: "branches native-key-bindings", outlet: "branches" });
			});
			return this.div({ class: "buttons" }, () => {
				...
				return this.button({ click: "cancel" }, () => {
					this.i({ class: "icon x" });
					this.span("Cancel");
				});
			});
	...
	fetch() {
		...
		git.fetch(this.root)
			...
			.then(branches => {
				...
				return this.switchButton.prop({ disabled: false });
			});
	}
};
