...
export default class FileTree {
	...
	render() {
		...
		const renderItems = (files) => {
			return files.map(obj => {
				...
				if (obj.file) {
					...
					li = (
						<li key={obj.key} className={classes.join(" ")} title={obj.file.file}>
					...
					li = (
						<li key={obj.key} className={classes.join(" ")}>
		...
		const hasDirs = this.hasDirs();
		const showCheckAll = this.state.showCheckboxes && this.moreThanOne();

		return (
			<div className="file-tree">
				<div className="buttons">
					<button tabIndex={tabIndex++} className={hasDirs ? "" : "hidden"} on={{click: () => this.changeCollapsed(true)}}>Collapse All</button>
					<button tabIndex={tabIndex++} className={hasDirs ? "" : "hidden"} on={{click: () => this.changeCollapsed(false)}}>Expand All</button>
					<button tabIndex={tabIndex++} className={showCheckAll ? "" : "hidden"} on={{click: () => this.changeChecked(true)}}>Check All</button>
					<button tabIndex={tabIndex++} className={showCheckAll ? "" : "hidden"} on={{click: () => this.changeChecked(false)}}>Uncheck All</button>
				</div>
				<div className="files">
					<ul>
