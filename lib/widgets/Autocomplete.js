...
export default class Autocomplete {
	...
	getDefaultProps() {
		return {
			 ...
			 * @return {void}
			 */
			onInput(event, value) {}, // eslint-disable-line no-unused-vars
			 ...
			 * @return {void}
			 */
			onSelect(value, item) {}, // eslint-disable-line no-unused-vars
			 ...
			 * @return {void}
			 */
			onRemove(value, item) {}, // eslint-disable-line no-unused-vars
			...
			renderItem(value, isHighlighted, styles) {
				...
				const removeButton = (this.removeButton ? (
					<i className="icon icon-x clickable autocomplete-remove-button" title="Remove" on={{click: removeClick}}></i>
				) : null);
				return (
			 ...
			 * @return {void}
			 */
			onMenuVisibilityChange(isOpen) {}, // eslint-disable-line no-unused-vars
	...
	handleKeyDown(event) {
		const keyDownHandlers = {
			...
			Delete(e) {
				...
				if (this.props.removeButton && this.state.highlightedIndex !== null && e.shiftKey) {
	...
	handleChange(event, force = false) {
		...
		return (
			<div className="autocomplete">
				<input className="native-key-bindings input-text autocomplete-input" attributes={{
					...
					"aria-expanded": open,
					autoComplete: "off",
				}} tabIndex={this.props.tabIndex} ref="input" on={{focus: this.handleInputFocus, blur: this.handleInputBlur, change: this.handleChange, input: this.handleInput, keydown: this.handleKeyDown, click: this.handleInputClick}} value={this.state.value}/> {open ? this.renderMenu() : null}
			</div>
		);
	}
}
