...
export default class Autocomplete {
		...
		return {
			...
			renderItem(value, isHighlighted, styles) {
				const removeButton = (this.removeButton ? (
					<i className="icon icon-x clickable autocomplete-remove-button" title="Remove" onclick={() => this.onRemove(this.value, value)}></i>
				) : null);
				return (
	...
	handleKeyDown(event) {
		const keyDownHandlers = {
			...
			Delete(event) {
				// click remove button
				if (this.props.removeButton && this.state.highlightedIndex !== null) {
					this.props.onRemove(this.props.value, this.refs[`item-${this.state.highlightedIndex}`].textContent);
					event.preventDefault();
				}
			},
	...
	render() {
		...
		return (
			<div style={this.props.wrapperStyle} {...this.props.wrapperProps} className={className + " autocomplete"}>
				<input {...inputProps} className="native-key-bindings input-text autocomplete-input" attributes={{
					role: "combobox",
					"aria-autocomplete": "list",
					"aria-expanded": open,
					autoComplete: "off",
				}} tabIndex={this.props.tabIndex} ref="input" onfocus={this.handleInputFocus} onblur={this.handleInputBlur} onchange={this.handleChange} oninput={this.handleInput} onkeydown={this.composeEventHandlers(this.handleKeyDown, inputProps.onKeyDown)} onkeyup={this.composeEventHandlers(this.handleKeyUp, inputProps.onKeyUp)} onclick={this.composeEventHandlers(this.handleInputClick, inputProps.onClick)} value={this.props.value}/> {open ? this.renderMenu() : null}
			</div>
		);
	}
}
