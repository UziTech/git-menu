...
export default class RunCommandDialog extends Dialog {
	...
	commandRemoveItem(value, item) {
		...
		this.update({recentItems: this.getRecentItems()});
	}
		...
		return (
			<div>
				...
				<label className="input-label">
					...
					<Autocomplete items={this.state.recentItems} removeButton="true" onRemove={this.commandRemoveItem} maxItems="10"  ref="commandInput" tabIndex={ this.state.files.length + 1 } value={this.state.command} onInput={this.commandChange} />
				</label>
			</div>
		);
	}
