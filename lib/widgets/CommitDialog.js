...
export default class CommitDialog extends Dialog {
	...
	body() {
		...
		const messageTooLong = this.state.message.split("\n").some((line, idx) => ((idx === 1 && line.length > 50) || line.length > 80));
		...
		return (
			<div>
				...
				<label>
					Message
					<textarea ref="messageInput" tabIndex={this.state.files.length + 1} className={(messageTooLong ? "toolong " : "") + "message native-key-bindings"} oninput={this.messageChange} value={this.state.message}/>
				</label>
				<label>
	...
	buttons() {
		return (
			<div>
				<button tabIndex={this.state.files.length + 3} onclick={this.accept}>
					<i className="icon commit"></i>
					<span>Commit</span>
				</button>
				<button tabIndex={this.state.files.length + 4} onclick={this.pushClick}>
					<i className="icon push"></i>
					<span>Commit & Push</span>
				</button>
				<button tabIndex={this.state.files.length + 5} onclick={this.pushPullClick}>
					<i className="icon pull"></i>
					<span>Commit & Push & Pull</span>
				</button>
				<button tabIndex={this.state.files.length + 6} onclick={this.cancel}>
