...
export default class CommitDialog extends Dialog {
	...
	body() {
			...
			return (
				<div className={classes.join(" ")}>
					<label className="input-label">
						<input className="input-checkbox" type="checkbox" tabIndex={idx + 1} checked={file.selected} onchange={this.fileChange(idx)}/>
						{file.file}
					</label>
				</div>
			);
		...
		return (
			<div>
				...
				<label className="input-label">
					<input className="input-checkbox" type="checkbox" tabIndex={this.state.files.length + 2} checked={this.state.amend} onchange={this.amendChange} disabled={this.state.lastCommit === null}/>
	...
	buttons() {
		return (
			<div className="block">
				<button className="btn icon icon-git-commit inline-block-tight" tabIndex={this.state.files.length + 3} onclick={this.accept}>
					Commit
				</button>
				<button className="btn icon icon-repo-push inline-block-tight" tabIndex={this.state.files.length + 4} onclick={this.pushClick}>
					Commit & Push
				</button>
				<button className="btn icon icon-repo-pull inline-block-tight" tabIndex={this.state.files.length + 5} onclick={this.pushPullClick}>
					Commit & Push & Pull
				</button>
			</div>
		);
