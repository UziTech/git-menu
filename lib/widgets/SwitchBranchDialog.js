...
export default class SwitchBranchDialog extends Dialog {
	...
	buttons() {
		return (
			<div>
				<button className="native-key-bindings" tabIndex="2" onclick={this.accept} disabled={this.state.fetching}>
					<i className="icon branch"></i>
					<span>Switch Branch</span>
				</button>
				<button className="native-key-bindings" tabIndex="3" onclick={this.fetch} disabled={this.state.fetching}>
					<i className="icon sync"></i>
					<span>Fetch</span>
				</button>
				<button className="native-key-bindings" tabIndex="4" onclick={this.cancel}>
