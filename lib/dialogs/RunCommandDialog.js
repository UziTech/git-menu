...
export default class RunCommandDialog extends Dialog {
		...
		return (
			<div>
				...
				<label className="input-label">
					...
					<Autocomplete items={this.state.recentItems} removeButton="true" onRemove={this.commandRemoveItem} maxItems="10" ref="commandInput" tabIndex={1001} value={this.state.command} onChange={this.commandChange} />
				</label>
			</div>
		);
	}
	...
	buttons() {
		return (
			<div>
				<button className="native-key-bindings btn icon icon-dashboard inline-block-tight" tabIndex={1002} on={{click: this.accept}}>
					Run
				</button>
			</div>
		);
