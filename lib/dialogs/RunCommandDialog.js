			...
			return (
				<div className={classes.join(" ")}>
					<label className="input-label">
						<input className="native-key-bindings input-checkbox" type="checkbox" tabIndex={idx + 1} checked={file.selected} on={{change: this.fileChange(idx)}}/>
						{file.file}
					</label>
				</div>
			);
		...
		return (
			<div>
				...
				<label className="input-label">
					...
					<Autocomplete items={this.state.recentItems} removeButton="true" onRemove={this.commandRemoveItem.bind(this)} maxItems="10"  ref="commandInput" tabIndex={ this.state.files.length + 1 } value={this.state.command} onChange={this.commandChange.bind(this)} />
				</label>
			</div>
		);
	}
