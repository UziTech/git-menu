...
export default {
	...
	async command(filePaths, statusBar, git = gitCmd, notifications = Notifications, dialog = MergeBranchDialog, title = "Merge Branch") {
		...
		const [rootBranch, mergeBranch, rebase, deleteBranch] = await new dialog({branches, root}).activate();
		...
		if (rebase) {
			const rebaseResult = await git.rebase(root, mergeBranch.name);
			notifications.addGit(title, rebaseResult);
		} else {
