	...
	activate() {
		...
		this.disposables.add(atom.commands.add("atom-workspace", {
			"context-git:commit": (event) => {
				command.commit(event, self.statusBarManager);
			},
			"context-git:commit-all": (event) => {
				command.commitAll(event, self.statusBarManager);
			},
			"context-git:discard": (event) => {
				command.discard(event, self.statusBarManager);
			},
			"context-git:undo-last-commit": (event) => {
				command.undoLastCommit(event, self.statusBarManager);
			},
		}));
	},
