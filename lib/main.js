...
export default {
	...
	activate() {
		...
		this.disposables.add(atom.commands.add("atom-workspace", {
			...
			"context-git:discard-changes": (event) => {
				command(event, self.statusBarManager, "discardChanges");
			},
			"context-git:discard-all-changes": (event) => {
				command(event, self.statusBarManager, "discardAllChanges");
			},
			"context-git:add-to-last-commit": (event) => {
				command(event, self.statusBarManager, "addToLastCommit");
			},
			"context-git:undo-last-commit": (event) => {
				command(event, self.statusBarManager, "undoLastCommit");
			},
			"context-git:switch-branch": (event) => {
				command(event, self.statusBarManager, "switchBranch");
			},
