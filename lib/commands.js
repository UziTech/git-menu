...
const commands = {
					...
					.then(([
						...
						return git.add(root, filePaths)
							...
							.then(result => {
								...
								if (pull) {
									return commands["pull-&-push"](filePaths, statusBar, git, notifications)
										.then(_ => numFiles + " committed & pulled & pushed.");
								} else if (push) {
									return commands.push(filePaths, statusBar, git, notifications)
	...
	"pull-&-push": {
		...
		command(filePaths, statusBar, git = gitCmd, notifications = Notifications) {
			return commands.pull(filePaths, statusBar, git, notifications)
				.then(_ => commands.push(filePaths, statusBar, git, notifications))
