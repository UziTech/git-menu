...
const commands = {
	...
	"undo-last-commit": {
		...
		confirm: {
			...
			detailedMessage: function (filePaths, git = gitCmd) {
				return helper.getRoot(filePaths, git)
					.then(root => git.lastCommit(root))
					.then(lastCommit => "You are undoing the commit:\n" + lastCommit);
			}
		},
		command(filePaths, statusBar, git = gitCmd, notifications = Notifications) {
