...
const commands = {
	...
	"commit": {
		...
		command(filePaths, statusBar, git = gitCmd, dialog = CommitDialog) {
					...
					.then(([
						...
						return git.add(root, filePaths)
							...
							.then(result => {
								...
								if (pull) {
									return commands["pull-&-push"](filePaths, statusBar, git)
										.then(_ => numFiles + " committed & pulled & pushed.");
								} else if (push) {
									return commands.push(filePaths, statusBar, git)
	...
	"commit-all": {
		...
		command(filePaths, statusBar, git = gitCmd, dialog = CommitDialog) {
			return commands.commit.command(atom.project.getPaths(), statusBar, git, dialog);
		},
	},
	...
	"add-to-last-commit": {
		...
		command(filePaths, statusBar, git = gitCmd) {
	...
	"discard-all-changes": {
		...
		command(filePaths, statusBar, git = gitCmd) {
			return commands["discard-changes"].command(atom.project.getPaths(), statusBar, git);
		},
	},
	...
	"pull-&-push": {
		...
		command(filePaths, statusBar, git = gitCmd) {
			return commands.pull(filePaths, statusBar, git)
				.then(_ => commands.push(filePaths, statusBar, git))
	...
	"switch-branch": {
		...
		command(filePaths, statusBar, git = gitCmd, dialog = SwitchBranchDialog) {
			return helper.getRoot(filePaths, git).then(root => {
	...
	"create-branch": {
		...
		command(filePaths, statusBar, git = gitCmd, dialog = CreateBranchDialog) {
			return helper.getRoot(filePaths, git).then(root => {
				return git.branches(root)
					.then(branches => {
						return new dialog({ branches, root }).activate();
	...
	"ignore-changes": {
		...
		command(filePaths, statusBar, git, ignore = true) {
	...
	"unignore-changes": {
		...
		command(filePaths, statusBar, git = gitCmd) {
			return commands["ignore-changes"].command(filePaths, statusBar, git, false);
		},
	},
